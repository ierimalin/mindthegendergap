...
lrgen$regime[lrgen$country %in% c("Austria", "Australia", "Belgium", "Canada", "Czech Republic", "Denmark", "Estonia", "Finland", "France", "Germany", "Greece", "Latvia", "United Kingdom", "Iceland", "Ireland", "Italy", "Luxembourg", "The Netherlands", "Norway", "Portugal", "Slovakia", "Slovenia", "Spain", "Sweden", "Switzerland", "Israel", "Chile", "Mexico", "New Zealand", "Japan", "United States", "South Korea")] <- 3
...
# Latvia
lrgen$regime[lrgen$country == "Latvia" & (lrgen$year <= 2009 | lrgen$year == 2013 | lrgen$year == 2016)] <- 2
...
`Female` is the main predictor variable. It is measured on a binary scale, with 0 representing male, and 1 representing female. As gender acts as the proxy for sociocultural factors and is the main predictor variable of interest, we remove all observations with missing gender values from the data set.[^3]

[^3]: In the surveys used for this data set, sex was often directly coded by interviewers, rather than inquired from respondents themselves: "In political matters people talk of "the left" and "the right". How would you place your views on this scale?"[@european_social_survey_ess_2004; @world_values_survey_2017-2021_2021]
...
plot(2004:2018, female_ideology_year,
     main = "Figure 3. Development of average female and male \n ideological self-placement over time",
...
mtext("Figure 5. Development of female (red) and male (blue) ideological self-placement over time, by country", side = 3, line = 0, cex=3.5, outer = TRUE)
...
mtext("Figure 6. Development of difference in female and male ideological self-placement over time", side = 3, line = 0, cex = 4.5, outer = TRUE)
...
# # Finding: Small negative effect of gender on ideology (females slightly more left than males) : Finding supports H1
# fit1 <- lm(lrgen$ideology_st ~ lrgen$female * lrgen$country + lrgen$year)
# # WORKING NOTE - Model WITH FIXED EFFECTS: fit1 <- lm(lrgen$ideology_st ~ lrgen$female + lrgen$year + lrgen$country)
# 
# ## regression of ideology on gender interacting with year
# # Finding: Over time, negative effect of gender on ideology increases (today, effect of females being more left than males is stronger than in the past) : Finding supports H2?
...
# # WORKING NOTE - Model WITH FIXED EFFECTS: fit2 <- lm(lrgen$ideology_st ~ lrgen$female * lrgen$year + lrgen$country)
# 
# ## regression of ideology on gender interacting with age
# # Finding: Effect of gender on ideology decreases with age (larger gender-differences for younger people): moderating effect of age
# fit3 <- lm(lrgen$ideology_st ~ lrgen$female * lrgen$age)
# # WORKING NOTE - Model WITH FIXED EFFECTS: fit3 <- lm(lrgen$ideology_st ~ lrgen$female * lrgen$age + lrgen$year + lrgen$country)
# 
...
# # WORKING NOTE - Model WITH FIXED EFFECTS: fit5 <- lm(lrgen$ideology_st ~ lrgen$female * lrgen$country + lrgen$year)
# 
...
# #fit6 = lm(ideology_st ~ female + regime + year, data = lrgen)
# # WORKING NOTE - Model WITH FIXED EFFECTS: fit6 = lm(ideology_st ~ female * regime + year + country, data = lrgen)
# 
...
# regression table
# 
# stargazer::stargazer(fit1, fit3, fit5, fit6,
#                       ## Title of the table
...
The $\beta1$ coefficient indicates that, on average, being a woman is associated with slightly more liberal self-identification (``` r round(``coef(proto_fit_1_stats)["lrgen$female"],2) ```. Despite the fact the result is statistically significant, we believe that the effect might be moderated by the country in which the survey had taken place. In other words, the difference between women and men's self-reported ideology might vary from country to country depending on cultural, political, socioeconomic or women rights-related factors. Therefore, we are adding to the simple model the interaction term between gender and country.
...
The $\beta3$ coefficient is very statistically significant and has a value of `r round(coef(fit3)["lrgen$female:lrgen$age"],3)`, indicating that women's ideology tends to change more than that of men, as it is bigger than $\beta2$ (`r  round(coef(fit3)["lrgen$age"],3)`). Essentially, this shows that the gender gap in ideological self-placement widens as women age too. **(did i interpret it right? it should show that the effect is slightly stronger for females, no?**).

We can conclude Hypothesis 1 is passed.

\newpage

## Hypothesis 2

For the second hypothesis, we aim to determine whether the gender-ideology relationship strenghtens over time in our timeframe. Naturally, we begin by interacting gender and year to explore the potential changes in their relationship over the years.
...
```{r}
#the weirdest thing happens. I can't add this text without getting an error when knitting.
# The analysis shows that, in the theoretical reference year, women have a self-reported ideology score higher than males (r round(coef(proto_fit_2a)[lrgen$female],2) units over r round(coef(proto_fit_2a)["(Intercept)"],2)).
# 
#  $\beta2$ shows that both genders tend to become more liberal over time (r round(coef(proto_fit_2a)["lrgen$year"],2)), however $\beta3$ confirms our expectations that this trend is more pronounced for women (r round(coef(proto_fit_2a)["lrgen$female:lrgen$year"],2)).
```

As previously established, adding countries as a fixed term helps us control for the inherent differences there might be between countries. Thus, this is the final model:
